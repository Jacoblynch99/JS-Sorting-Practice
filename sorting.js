...
// Needs to be able to loop over an entire array and take the string data type and convert it to the number data type, and then add the result to a new array
const stringsToNumbs = (num) => {
  for (let i = 0; i < num.length; i++) {
    arrayOfNums.push(parseInt(num[i], 10))
  }
}
stringsToNumbs(strNums)

// With the same numbers, find the sum of the even values
// take the number data type array and loop over it, and target even values with an if statement
// should also set up a bucket called sum to add all the values in
const sumEvens = (num) => {
  let sum = 0
  for (let i = 0; i < num.length; i++){
    if (num[i] % 2 === 0){
...
sumEvens(arrayOfNums)

// Find the index of the first value when added to it's index = 512 (#ATX!!)
// Should loop through the array looking at both index and value and adding them.
// When a pair hits the sum total of 512, return the console log
const atxIdx = (num) => {
  for (let i = 0; i < num.length; i++){
    if (num[i] + i === 512) {
      return console.log(`index: ${i}, value: ${num[i]}`)
    }
  }
  return "Found It!"
}
...
const uniqueWeatherState = []

const weatherStates = (arr) => {
  for (let i = 0; i < arr.length; i++) {
    arr.forEach(() => {
      if (!uniqueWeatherState.includes(arr[i].weather_state_name)) {
        uniqueWeatherState.push(arr[i].weather_state_name);
      }
    })
  }
}
weatherStates(weather)
console.log(uniqueWeatherState)
